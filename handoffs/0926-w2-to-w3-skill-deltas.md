...
Corollary, from a metric built in this workstream and wrong: **frequency is not evidence.** A depth metric grading EST by *what share of a company's postings mention estate work* ranked **Vinted "thin" at 3%** and another company **"substantive" at 90%** on *"our platform saw only 6 minutes of downtime."* Rate measures how infrastructure-flavoured a hiring corpus is. It does not measure evidence.

**Correction, 2026-09-04 — the example this section used to carry was itself the defect it describes.** Vinted was cited here as carrying *"Scaling database clusters (Vitess, MySQL) by introducing new sharding strategies"*, called "the single most specific estate sentence in the corpus." **That sentence does not exist in any data file.** It appears only in this workstream's own three write-ups. The real corpus text is *"...has managed database platforms at scale and understands the tradeoffs behind systems like Vitess, CockroachDB, Spanner, DynamoDB, Cassandra, Redis, ClickHouse..."* — **a candidate requirement, not an estate description**, which is precisely item 5. Vinted's `EST●` in the undated pull rested on a `[reliability/toil]` match on the bullet *"You avoid TOIL"* — a candidate attribute, on two of the terms item 4 says to exclude at 67% and 31% prevalence. The dated pull reads **`EST○` — absent across 67 requisitions**, and is right.

**So the argument stands and the evidence for it was fabricated by paraphrase.** Eighth instance of the family, first in which the defect is in the passage arguing against it. The demonstration that frequency and evidence quality are orthogonal does not need an invented quote: **Housecall Pro carries 36 pain postings in 12 months of 56** and a real system-decomposition sentence; **Cover Genius carries 1 of 1** and its single match is a duties-list bullet; **Zeta carried 1 of 1** on `legacy` used as heritage. High frequency with real evidence, low frequency with weak evidence, low frequency with spurious evidence — three real accounts, three combinations, no paraphrase.
