...
## Tier 1 — score ≥56.25  (2 accounts)

Named-account ABM. Run `/abm-campaign` per account.

| Account | Score /75 | 7-signal | Gates (AGE · SCL · VOL are gates only — they no longer score) | Trigger evidence | Cohort |
|---|---|---|---|---|---|
| **Sensor Tower** · `sensortower.com` | 75 | 77 | `VER◐ AGE? SCL◐ VOL? EST● MRG● HIR● ACC● MDB●` | **MongoDB present**; named estate pain in an open req; margin/sponsor trigger confirmed; live infra hiring; MongoDB confirmed — direction checked, neutral; `AGE?` Founded Year blank | B — sponsor mandate |
| **Close** · `close.com` | 65 | 65 | `VER◐ AGE? SCL○ VOL? EST● MRG◐ HIR● ACC● MDB●` | **MongoDB present**; named estate pain in an open req; live infra hiring; MongoDB confirmed — direction checked, neutral; `AGE?` Founded Year blank | A — hiring trigger |

**Close reaches Tier 1, three days after the disagreement was opened.** `0926-target-accounts.md` recorded on 2 September that the model scored Close 80 against a hand rank of 29, and that "one of the two is wrong, and the model is at least legible." **The model was right.** Close was hand-ranked down on unverified scale — and scale is one of the three signals now proven not to discriminate. Its `SCL○` no longer costs it anything, and what remains is real: MongoDB confirmed on product-stack detection and direction-checked, named estate pain in an open requisition, live infra hiring from a P1 posting with a genuine date.

## Tier 2 — score 37.5–56.24  (4 accounts)

Persona sequences and signal monitoring, not named-account ABM.

| Account | Score /75 | 7-signal | Gates (AGE · SCL · VOL are gates only — they no longer score) | Trigger evidence | Cohort |
|---|---|---|---|---|---|
| **Owner.com** · `owner.com` | 50 | 65 | `VER● AGE● SCL● VOL? EST● MRG◐ HIR● ACC● MDB⊘` | **MongoDB present**; named estate pain in an open req; live infra hiring; **`MDB⊘` — migration *away* from MongoDB named in its own requisition, co-sell weight zeroed (rule 5)** | A — hiring trigger |
| **Signifyd** · `signifyd.com` | 50 | 65 | `VER● AGE● SCL● VOL? EST● MRG◐ HIR● ACC✕ MDB○` | named estate pain in an open req; live infra hiring; access obstacle flagged | A — hiring trigger |
| **Alan** · `alan.com` | 40 | 55 | `VER● AGE● SCL● VOL? EST● MRG○ HIR● ACC◐ MDB○` | named estate pain in an open req; live infra hiring | A — hiring trigger |
| **NexHealth** · `nexhealth.com` | 37.5 | 37 | `VER◐ AGE? SCL✕ VOL? EST◐ MRG◐ HIR● ACC● MDB○` | partial estate signal; live infra hiring; `AGE?` Founded Year blank | A — hiring trigger |

## Tier 3 — score <37.5  (25 accounts)

Nurture and trigger-monitoring only. Not sequence-eligible.

| Account | Score /75 | 7-signal | Gates (AGE · SCL · VOL are gates only — they no longer score) | Trigger evidence | Cohort |
|---|---|---|---|---|---|
| **Zuora** · `zuora.com` | 35 | 50 | `VER◐ AGE● SCL● VOL? EST○ MRG● HIR○ ACC● MDB●` | margin/sponsor trigger confirmed; **legacy relational co-present**; MongoDB confirmed — **direction unverified, no requisition on file** | C — re-score as M1 |
| **Carta** · `carta.com` | 35 | 50 | `VER● AGE● SCL● VOL? EST○ MRG● HIR○ ACC● MDB●` | margin/sponsor trigger confirmed; MongoDB confirmed — **direction unverified, no requisition on file** | E — efficiency reset |
| **Pushpay** · `pushpay.com` | 35 | 40 | `VER◐ AGE? SCL● VOL? EST○ MRG● HIR○ ACC● MDB●` | margin/sponsor trigger confirmed; **legacy relational co-present**; MongoDB confirmed — **direction unverified, no requisition on file**; `AGE?` Founded Year blank | B — sponsor mandate |
| **Metropolis Technologies** · `metropolis.io` | 20 | 47 | `VER○ AGE● SCL● VOL? EST? MRG● HIR○ ACC● MDB○` | partial estate signal; margin/sponsor trigger confirmed; **`EST?` — the `EST◐` mark was dropped 2026-09-03: 0 posting rows, blank `Database Technology Mentions`, source untraceable. 12.5 pts removed. In the 44-domain pull.** | B — post-acquisition integration |
| **iCapital** · `icapital.com` | 27.5 | 42.5 | `VER● AGE● SCL● VOL? EST○ MRG● HIR○ ACC◐ MDB◑` | margin/sponsor trigger confirmed; **legacy relational co-present**; *job-posting evidence only*; **`MDB◑` — MongoDB appears only as one slug in a 317-entry job-advert aggregate; the product-stack detector returned nothing. Half credit, and direction is unknowable from this source** | C — re-score as M1 |
| **FreedomPay** · `freedompay.com` | 27.5 | 40 | `VER● AGE● SCL◐ VOL? EST○ MRG● HIR◐ ACC◐ MDB?` | margin/sponsor trigger confirmed; **MongoDB UNKNOWN — detector returned no primary database** | B — sponsor mandate |
| **Workrise (now RigUp)** · `workrise.com` | 27.5 | 32.5 | `VER◐ AGE? SCL● VOL? EST○ MRG● HIR○ ACC● MDB◑` | margin/sponsor trigger confirmed; *job-posting evidence only*; `AGE?` Founded Year blank; **`MDB◑` — MongoDB appears only as one slug in a 314-entry job-advert aggregate; the product-stack detector returned nothing. Half credit, and direction is unknowable from this source** | E — efficiency mandate |
| **ID.me** · `id.me` | 25 | 40 | `VER◐ AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC✕ MDB●` | access obstacle flagged; **legacy relational co-present**; MongoDB confirmed — **direction unverified, no requisition on file** | D — watchlist, no trigger |
| **Zeta** · `zeta.tech` | 25 | 40 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB●` | firmographics only — no trigger; **legacy relational co-present**; MongoDB confirmed — **direction unverified, no requisition on file** | D — watchlist, no trigger |
| **ezCater** · `ezcater.com` | 25 | 40 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB●` | firmographics only — no trigger; MongoDB confirmed — **direction unverified, no requisition on file** | D — watchlist, no trigger |
| **Cover Genius** · `covergenius.com` | 25 | 40 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB●` | firmographics only — no trigger; MongoDB confirmed — **direction unverified, no requisition on file** | D — watchlist, no trigger |
| **Netradyne** · `netradyne.com` | 25 | 40 | `VER◐ AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB●` | firmographics only — no trigger; **legacy relational co-present**; MongoDB confirmed — **direction unverified, no requisition on file** | D — watchlist, no trigger |
| **Zūm** · `ridezum.com` | 20 | 35 | `VER◐ AGE● SCL● VOL? EST○ MRG● HIR○ ACC● MDB○` | margin/sponsor trigger confirmed | B — sponsor mandate |
| **Cambridge Mobile Telematics** · `cmtelematics.com` | 20 | 25 | `VER● AGE? SCL● VOL? EST○ MRG● HIR○ ACC● MDB?` | margin/sponsor trigger confirmed; **MongoDB UNKNOWN — detector returned no primary database**; `AGE?` Founded Year blank | B — sponsor mandate |
| **Civitatis** · `civitatis.com` | 20 | 25 | `VER● AGE? SCL● VOL? EST○ MRG● HIR○ ACC● MDB○` | margin/sponsor trigger confirmed; `AGE?` Founded Year blank | B — sponsor mandate |
| **Weee!** · `sayweee.com` | 10 | 25 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger | D — watchlist, no trigger |
| **Housecall Pro** · `housecallpro.com` | 10 | 25 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger | D — watchlist, no trigger |
| **Back Market** · `backmarket.com` | 10 | 25 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger | D — watchlist, no trigger |
| **Blockchain.com** · `blockchain.com` | 10 | 25 | `VER● AGE● SCL● VOL? EST○ MRG◐ HIR○ ACC◐ MDB○` | firmographics only — no trigger | D — watchlist, no trigger |
| **Fleetio** · `fleetio.com` | 10 | 15 | `VER◐ AGE? SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger; `AGE?` Founded Year blank | D — watchlist, no trigger |
| **Wallapop** · `wallapop.com` | 10 | 15 | `VER● AGE? SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger; `AGE?` Founded Year blank | D — watchlist, no trigger |
| **Engine (formerly Hotel Engine)** · `hotelengine.com` | 10 | 15 | `VER● AGE? SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB○` | firmographics only — no trigger; `AGE?` Founded Year blank | D — watchlist, no trigger |
| **Sure** · `sureapp.com` | 10 | 15 | `VER● AGE? SCL● VOL? EST○ MRG◐ HIR○ ACC● MDB?` | firmographics only — no trigger; **MongoDB UNKNOWN — detector returned no primary database**; `AGE?` Founded Year blank | D — watchlist, no trigger |
| **Lighthouse** · `mylighthouse.com` | 0 | 15 | `VER● AGE● SCL● VOL? EST○ MRG○ HIR○ ACC● MDB?` | firmographics only — no trigger; **MongoDB UNKNOWN — detector returned no primary database** | D — watchlist, no trigger |
| **FarEye** · `fareye.com` | 0 | 15 | `VER● AGE● SCL● VOL? EST○ MRG○ HIR○ ACC◐ MDB?` | firmographics only — no trigger; **MongoDB UNKNOWN — detector returned no primary database** | D — watchlist, no trigger |
