...
| Company age | **Founded 2018 or earlier** — the sourcing proxy for *MVP-era architecture*, which requires elapsed time. A **minimum** age only; there is deliberately no upper bound (see note below) |
...
**The age test is a floor, not a range (derived, 2026-09-03).** "Founded 2018 or earlier" operationalizes *MVP-era architecture* for sourcing tools that cannot inspect an estate. It rejects companies too young to have accumulated a founding-era monolith. It deliberately has **no upper bound** — an older company genuinely running a digital platform often has the deepest estate and is the stronger target. The consequence matters: **age never excludes a pre-digital company; the digital-native qualifier does.** Drop that qualifier from a sourcing prompt and the age gate happily admits a law firm founded in 1748 and a credit union founded in 1931 — both returned by a real search on 3 September 2026. The qualifier is load-bearing and belongs in every prompt.
...
- **Also exclude (derived, 2026-09-02):** companies whose own product is infrastructure (databases, hosting, PaaS, developer platforms, observability, data connectivity, GPU compute, workflow orchestration) — deepest in-house benches, lowest winnability · portfolio/holding companies operating independent brands — no single estate, no single margin owner · publicly listed and SPAC-bound companies — ICP stops at pre-IPO · **(added 2026-09-03, from a 25-row database-estate search where 21 failed)** consultancies, systems integrators and professional-services firms — engineering, architecture, legal, staffing; they bill hours and run no transaction estate · hardware and industrial manufacturers — the estate is not the product · traditional non-digital-native financial institutions — mutual insurers, credit unions, annuity runoff books; real transaction volume but no MVP-era platform to modernize · captive shared-service centres and country subsidiaries of public parents — no local buying committee, no local margin owner · state-owned utilities and public bodies
